...
export function Editor({ documentId, content, saveState, onChange }: EditorProps) {
  ...
  const editor = useEditor({ extensions: [StarterKit], content, editorProps: { attributes: { role: 'textbox', 'aria-label': 'Document editor', 'aria-multiline': 'true', 'data-testid': 'tiptap-editor' } }, onUpdate: ({ editor: current }) => onChangeRef.current(current.getHTML()) })
  ...
  return <div><div role="toolbar" aria-label="Text formatting" className="mb-5 flex flex-wrap items-center gap-1 border-b border-stone-200 pb-3">{tools.map(({ label, icon: Icon, action, active }) => <button key={label} type="button" aria-label={label} title={label} data-active={active} onClick={action} className={`rounded p-2 ${active ? 'bg-emerald-100 text-emerald-900' : 'hover:bg-stone-100'}`}><Icon size={17} /></button>)}<span aria-live="polite" data-testid="save-status" className={`ml-auto font-sans text-xs ${saveState === 'offline' ? 'text-amber-700' : 'text-stone-500'}`}>{status}</span></div><EditorContent editor={editor} /></div>
}